...
local purelua = require("luarocks.build.purelua")
...
local function fill_build_data(build_system, rockspec, libs)
    local build_systems_map = {
        purelua = purelua,
        deafult = builtin
    }
    local build_system_module = build_systems_map[build_system] or build_systems_map["deafult"]
    rockspec.build = build_system_module.get_default_build_config(rockspec, libs) 
end
...
function write_rockspec.command(flags, name, version, url_or_dir)
    ...
    -- get build type for rockspec format
    local build_system = type_rockspec.default_build_system(rockspec_format)
    fill_build_data(build_system, rockspec, libs)
